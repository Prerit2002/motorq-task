...
router.post("/registration/:attendeeId/:eventId", async (req, res) => {
  ...
  try {
    ...
    console.log(event);
    if (attendee && event) {
      const allotted = await Allotted.findOne({
      ...
      await Promise.all(
        myEvents.map(async (ev) => {
          ...
          if (regEvent.day === requiredEvent.day) {
            if (
              (regEvent.startTime >= requiredEvent.startTime &&
                regEvent.endTime <= requiredEvent.endTime) ||
              (reqEvent.endTime >= requiredEvent.startTime &&
                reqEvent.endTime <= requiredEvent.endTime)
            ) {
              check = true;
            }
          }
